...
No demo or vignette was found for the package.
...
I saw three historgrams the first time I did this, but this time I'm seeing three boxplots. The median body masses of Chinstrap and Adelie penguins are lower than that of Gentoo penguins. The body mass of Gentoo penguines have greater variability than the other groups'. In the dataset for the Chinstrap group, there are 2 outliers.
...
For both part a and b: I can't use `seq(...)` because the distance between 2 observations in this vector is not fixed. The numbers in this sequence are also not repeated, so I can't use `rep(...)`. 
...
The resulting class is character because vectors can only contain one type of data, and the character class is further to the right than the numeric class, so 240 is coerced into "240".